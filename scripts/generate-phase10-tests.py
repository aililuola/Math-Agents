#!/usr/bin/env python3
"""Generate one JUnit wrapper for every phase-10 Python authority test module."""
...
def main() -> None:
    with STATE.open(encoding="utf-8-sig", newline="") as stream:
        rows = [row for row in csv.DictReader(stream) if row["primary_phase"] == "10"]
    if len(rows) != 46:
        raise SystemExit(f"expected 46 phase-10 test rows, found {len(rows)}")
    ...
    for row in rows:
        ...
        content = f"""package io.github.aililuola.mathproofmesh.proofcontrol;
...
class {class_name} {{
  ...
  void preservesAuthoritySemantics(String authorityFunction) {{
    ProofControlParityScenarios.verify("{class_name}", authorityFunction);
  }}
}}
"""
        (OUTPUT / f"{class_name}.java").write_text(content, encoding="ascii")

    if function_count != 203:
        raise SystemExit(f"expected 203 phase-10 functions, found {function_count}")
