#!/usr/bin/env python3
"""Generate one JUnit wrapper for every phase-12 authority test module."""
...
def main() -> None:
    with STATE.open(encoding="utf-8-sig", newline="") as stream:
        rows = [
            row for row in csv.DictReader(stream) if row["primary_phase"] == "12"
        ]
    if len(rows) != 18:
        raise SystemExit(f"expected 18 phase-12 test rows, found {len(rows)}")
    ...
    for row in rows:
        ...
        content = f"""package io.github.aililuola.mathproofmesh.orchestration;
...
class {class_name} {{
  ...
  void preservesAuthoritySemantics(String authorityFunction) {{
    OrchestrationParityScenarios.verify("{class_name}", authorityFunction);
  }}
}}
"""
        (OUTPUT / f"{class_name}.java").write_text(content, encoding="ascii")

    if function_count != 107:
        raise SystemExit(f"expected 107 phase-12 functions, found {function_count}")
