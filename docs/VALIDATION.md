...
| `python -m pytest -q` | **244 passed**（安装 `.[dev,server]`，包含 `z3-solver`） |
...
当前包含上述历史回归在内的完整自动化结果为 `244 passed`。分段确定性完整演示结果仍为 `verified`，25 calls，32,920 tokens。所有路线数、动作数、修补次数和调用预算均来自配置，不绑定某一道题。
...
pytest: 244 passed
ruff check: PASS
ruff format --check: PASS (174 files)
compileall: PASS
