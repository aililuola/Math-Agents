...
(() => {
  ...
  class TopologyView {
    ...
    storeEvent(key, event) {
      ...
      const normalized = {
        ...
        parent_task_id:
          event.parent_task_id || previous?.parent_task_id || null,
    ...
    renderNode(node) {
      ...
      head.append(
        htmlElement("span", "topology-node-status"),
        htmlElement(
          "span",
          "topology-node-title",
        ...
        htmlElement(
          "span",
          "topology-node-time",
          formatElapsed(node.event.elapsed_ms),
        ),
      );
      const detail = htmlElement(
        "div",
        "topology-node-detail",
        node.event.detail || nodeTypeLabel(node),
      );
      ...
      meta.append(
        htmlElement(
          "span",
          "topology-node-agent",
          node.event.agent_id || nodeTypeLabel(node),
        ),
        htmlElement(
          "span",
          "topology-node-stage",
          friendlyStage(node.event.stage),
        ),
      );
  ...
  function nodeKind(event) {
    ...
    if (initialType === "run" || initialType === "run_resume") {
      return "run";
    }
    if (
      [
