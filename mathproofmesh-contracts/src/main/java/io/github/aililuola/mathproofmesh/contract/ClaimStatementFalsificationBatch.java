...
public record ClaimStatementFalsificationBatch(
    ...
    @JsonProperty(value = "decisions") @ContractNonNull
        List<ClaimStatementFalsificationDecision> decisions,
    @JsonProperty(value = "raw_artifact_ref") String rawArtifactRef,
    @JsonProperty(value = "usage") @ContractNonNull UsageRecord usage)
    implements StrictContract {
  public ClaimStatementFalsificationBatch {
    if (batchId == null) {
      batchId = PythonCompatibleIdGenerator.newId("statement-falsification");
    }
    ...
    for (ClaimStatementFalsificationDecision decision : decisions) {
      if (!claimIds.add(decision.claimId())) {
        throw new ContractValidationException(
            "duplicate statement falsification decision: " + decision.claimId());
      }
    }
  ...
  @Override
  public List<ClaimStatementFalsificationDecision> decisions() {
