...
public record ClaimBlindAdjudicationBatch(
    ...
    @JsonProperty(value = "decisions") @ContractNonNull
        List<ClaimBlindAdjudicationDecision> decisions,
    @JsonProperty(value = "raw_artifact_ref") String rawArtifactRef,
    @JsonProperty(value = "usage") @ContractNonNull UsageRecord usage)
    implements StrictContract {
  public ClaimBlindAdjudicationBatch {
    if (batchId == null) {
      batchId = PythonCompatibleIdGenerator.newId("claim-blind-adjudication");
    }
    ...
    for (ClaimBlindAdjudicationDecision decision : decisions) {
      if (!claimIds.add(decision.claimId())) {
        throw new ContractValidationException(
            "duplicate blind adjudication decision: " + decision.claimId());
      }
    }
  ...
  @Override
  public List<ClaimBlindAdjudicationDecision> decisions() {
