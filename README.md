...
本轮离线验收基线为：安装 `.[dev,server]`（包含 `z3-solver`）后 `604 passed`、Ruff check 通过、Ruff format check 通过、`compileall` 通过、topology 与 proof-control component-contract Mock benchmark 通过且真实 provider 调用数为 0。这些 Mock benchmark 验证组件契约和消融开关，不代表真实 IMO 求解性能。
