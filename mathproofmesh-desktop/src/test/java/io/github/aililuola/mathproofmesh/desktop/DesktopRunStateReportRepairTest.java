...
final class DesktopRunStateReportRepairTest {
  ...
  void reportRepairChangesOnlyProjectionHash() {
    ...
    RunStateSnapshot failed = projection(authority, RunReportStatus.PROJECTION_FAILED, "", 0L);
    FileRunStateStore store = new FileRunStateStore(temporaryDirectory);
    store.compareAndSet("report-repair", -1, failed, "test", 0);
    RunStateSnapshot repaired =
        projection(authority, RunReportStatus.PARTIAL, "reports/run_report.md", 1L);
    store.compareAndSetProjection(
        "report-repair", failed.stateHash(), 0L, repaired, "test", 0);
  ...
  private static RunStateSnapshot projection(
      RunStateSnapshot source, RunReportStatus status, String reference, long projectionVersion) {
